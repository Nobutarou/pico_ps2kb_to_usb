...
//--------------------------------------------------------------------+
// ここに好きなキーを追加たり削除したり、偽装したり
// ただし装飾キーは HID の標準のコードではなく、このコード専用になってる
// 詳しくは static uint8_t get_modifier_mask(uint8_t hid_code) 参照
//--------------------------------------------------------------------+
...
static uint8_t get_modifier_mask(uint8_t hid_code) 
{
   switch (hid_code) {
      ...
      // 本当に caps lock が嫌いんんだろうな、というのが伝わってくる。
